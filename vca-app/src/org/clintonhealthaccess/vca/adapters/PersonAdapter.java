...
public class PersonAdapter extends ArrayAdapter<Person> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (p != null) {
			...
			if (textView != null) {
				textView.setText(p.getAge().toString());
			}
			...
			textView.setText(this.getContext().getString(R.string.name) + ": " +p.getName());
			
			textView = (TextView) v.findViewById(R.id.infoc_text);
			if(p.getPasive()=='1') {
				textView.setTextColor(Color.RED);
				textView.setText("Esta persona está inactiva");
			}
			else {
				textView.setText("");
			}
			...
			if (imageView != null) {
				if (String.valueOf(p.getSex()).equals("M")) {
					imageView.setImageResource(R.drawable.male);
				} else if (String.valueOf(p.getSex()).equals("F")) {
