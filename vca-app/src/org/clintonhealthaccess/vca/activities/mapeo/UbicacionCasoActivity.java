...
public class UbicacionCasoActivity extends AbstractAsyncActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		infoMovil = new DeviceInfo(UbicacionCasoActivity.this);
		
		//Aca se recupera los datos del caso y la localidad
		...
		textViewCaso = (TextView) findViewById(R.id.label_encuesta);
		...
		textViewHeader.setText("Ubicación lugar de detección del caso");

		
		textViewCaso.setText(getString(R.string.codeCase)+":"+mCaso.getCodigo()+"\n"+
				getString(R.string.localidad)+":"+mCaso.getLocal().getName()
					+"\n\n"+
				" Datos actuales:\n");
		if(mCaso.getLatitude()==null||mCaso.getLatitude()==0) {
			textViewLatitud.setText("Latitud no establecida");
		}else{
			textViewLatitud.setText("Latitud: " + mCaso.getLatitude().toString());
		}
		
		if(mCaso.getLongitude()==null||mCaso.getLongitude()==0) {
			textViewLongitud.setText("Longitud no establecida");
		}else{
			textViewLongitud.setText("Longitud: "+mCaso.getLongitude().toString());
		}
		
		if(mCaso.getExactitud()==null||mCaso.getExactitud()==0) {
			textViewExactitud.setText("Proximidad no establecida");
		}else{
			textViewExactitud.setText("Proximidad: "+mCaso.getExactitud().toString());
		}
		
		if(mCaso.getAltitud()==null||mCaso.getAltitud()==0) {
			textViewAltitud.setText("Altitud no establecida");
		}else{
			textViewAltitud.setText("Altitud: "+mCaso.getAltitud().toString());
		}
	...
	// ***************************************
	 	...
	 	private class SaveDataTask extends AsyncTask<String, Void, String> {
	 		...
	 		protected String doInBackground(String... values) {
	 			...
	 			try {
	 				vcaAdapter.open();
	 				mCaso.setDeviceid(infoMovil.getDeviceId());
	 				mCaso.setLatitude(latitud);
	 				mCaso.setLongitude(longitud);
	 				mCaso.setAltitud(altitud);
	 				mCaso.setExactitud(exactitud);
	 				mCaso.setEstado(Constants.STATUS_NOT_SUBMITTED);
