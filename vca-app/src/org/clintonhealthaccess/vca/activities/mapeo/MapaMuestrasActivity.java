...
public class MapaMuestrasActivity extends AbstractAsyncActivity {
    MapView map = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        new FetchTestsTask().execute(strFiltro);
	...
	public void onBackPressed (){
		finish();
		Bundle arguments = new Bundle();
		Intent i = new Intent(getApplicationContext(), MuestrasActivity.class);
		if (mLocalidad != null) arguments.putSerializable(Constants.LOCALIDAD, mLocalidad);
		arguments.putString(Constants.ROLES, roles);
		i.putExtras(arguments);
		i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		...
		private class FetchTestsTask extends AsyncTask<String, Void, String> {
			...
			protected String doInBackground(String... values) {
				...
				try {
					vcaAdapter.open();
					mMuestras = vcaAdapter.getMuestras(filtro, MainDBConstants.mxDate);
					vcaAdapter.close();
				} catch (Exception e) {
		...
		// Private classes
		// ***************************************
		private class FetchTestTask extends AsyncTask<String, Void, String> {
			@Override
			protected void onPreExecute() {
				// before the request begins, show a progress indicator
				showLoadingProgressDialog();
			...
			@Override
			protected String doInBackground(String... values) {
				String strMuestra = values[0];
				String filtro = MainDBConstants.casa + " = '"+ strMuestra + "'";
				
				
				try {
					vcaAdapter.open();
					mMuestra = vcaAdapter.getMuestra(filtro, null);
					vcaAdapter.close();
				} catch (Exception e) {
					Log.e(TAG, e.getLocalizedMessage(), e);
					return "error";
			...
			protected void onPostExecute(String resultado) {
				dismissProgressDialog();
				showPreguntaAbrirMuestra(resultado);
			}
			...
			if(resultado.equals("exito")) {			
				if(mLocalidad==null) {
					...
					for(Muestra muestra:mMuestras) {
			        	if(muestra.getLatitude()!=null && muestra.getLongitude()!= null) {
			        		points.add(new LabelledGeoPoint(muestra.getLatitude(),muestra.getLongitude()
			        			, muestra.getCasa()));
			        	}
			        }
			        ...
			        sfpo.setOnClickListener(new SimpleFastPointOverlay.OnClickListener() {
			        	...
			        	public void onClick(SimpleFastPointOverlay.PointAdapter points, Integer point) {
			        		new FetchTestTask().execute(((LabelledGeoPoint) points.get(point)).getLabel());
			        	}
			        });
		...
		private void showPreguntaAbrirMuestra(String resultado) {
			if(resultado.equals("exito")) {			
