...
public class MapaPuntosDxActivity extends AbstractAsyncActivity {
    MapView map = null;
	...
	private List<PuntoDiagnostico> mPuntos = new ArrayList<PuntoDiagnostico>();
	private PuntoDiagnostico mPunto = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        new FetchPuntosTask().execute(strFiltro);
	...
	public void onBackPressed (){
		finish();
		Bundle arguments = new Bundle();
		Intent i = new Intent(getApplicationContext(), PuntosDxActivity.class);
		if (mLocalidad != null) arguments.putSerializable(Constants.LOCALIDAD, mLocalidad);
		arguments.putString(Constants.ROLES, roles);
		i.putExtras(arguments);
		i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		...
		// Private classes
		// ***************************************
		private class FetchPuntosTask extends AsyncTask<String, Void, String> {
			@Override
			...
			protected String doInBackground(String... values) {
				...
				try {
					vcaAdapter.open();
					mPuntos = vcaAdapter.getPuntosDiagnosticos(filtro, MainDBConstants.clave);
					vcaAdapter.close();
				} catch (Exception e) {
		...
		// Private classes
		// ***************************************
		private class FetchPuntoTask extends AsyncTask<String, Void, String> {
			@Override
			protected void onPreExecute() {
				// before the request begins, show a progress indicator
				showLoadingProgressDialog();
			...
			@Override
			protected String doInBackground(String... values) {
				String strClave = values[0];
				String filtro = MainDBConstants.clave + " = '"+ strClave + "'";
				
				
				try {
					vcaAdapter.open();
					mPunto = vcaAdapter.getPuntoDiagnostico(filtro, null);
					vcaAdapter.close();
				} catch (Exception e) {
					Log.e(TAG, e.getLocalizedMessage(), e);
					return "error";
			...
			protected void onPostExecute(String resultado) {
				dismissProgressDialog();
				showPreguntaAbrirPunto(resultado);
			}
		...
		private void showResult(String resultado) {
			if(resultado.equals("exito")) {			
				if(mLocalidad==null) {
					...
					for(PuntoDiagnostico punto:mPuntos) {
			        	if(punto.getLatitude()!=null && punto.getLongitude()!= null) {
			        		points.add(new LabelledGeoPoint(punto.getLatitude(),punto.getLongitude()
			        			, punto.getClave()));
			        	}
			        }
			        ...
			        sfpo.setOnClickListener(new SimpleFastPointOverlay.OnClickListener() {
			        	...
			        	public void onClick(SimpleFastPointOverlay.PointAdapter points, Integer point) {
			        		new FetchPuntoTask().execute(((LabelledGeoPoint) points.get(point)).getLabel());
			        	}
			        });
		...
		private void showPreguntaAbrirPunto(String resultado) {
			if(resultado.equals("exito")) {			
