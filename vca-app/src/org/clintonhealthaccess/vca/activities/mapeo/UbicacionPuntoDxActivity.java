...
public class UbicacionPuntoDxActivity extends AbstractAsyncActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		infoMovil = new DeviceInfo(UbicacionPuntoDxActivity.this);
		...
		textViewHeader.setText("Ubicación punto de diagnóstico");
		
		textViewExactitud.setVisibility(View.GONE);
		textViewAltitud.setVisibility(View.GONE);

		
		textViewPunto.setText(getString(R.string.clave)+":"+mPunto.getClave()+"\n"+
				getString(R.string.localidad)+":"+mPunto.getLocal().getName()
					+"\n\n"+
				" Datos actuales:\n");
		if(mPunto.getLatitude()==null||mPunto.getLatitude()==0) {
			textViewLatitud.setText("Latitud no establecida");
		}else{
			textViewLatitud.setText("Latitud: " + mPunto.getLatitude().toString());
		}
		
		if(mPunto.getLongitude()==null||mPunto.getLongitude()==0) {
			textViewLongitud.setText("Longitud no establecida");
		}else{
			textViewLongitud.setText("Longitud: "+mPunto.getLongitude().toString());
		}
	...
	// ***************************************
	 	...
	 	private class SaveDataTask extends AsyncTask<String, Void, String> {
	 		...
	 		protected String doInBackground(String... values) {
	 			...
	 			try {
	 				vcaAdapter.open();
	 				mPunto.setDeviceid(infoMovil.getDeviceId());
	 				mPunto.setLatitude(latitud);
	 				mPunto.setLongitude(longitud);
	 				mPunto.setEstado(Constants.STATUS_NOT_SUBMITTED);
	 				vcaAdapter.editarPuntoDiagnostico(mPunto);
	 				vcaAdapter.close();
