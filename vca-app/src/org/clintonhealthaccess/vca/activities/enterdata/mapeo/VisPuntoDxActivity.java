...
import org.clintonhealthaccess.vca.activities.mapeo.VisPuntosDxActivity;
...
import org.clintonhealthaccess.vca.forms.mapeo.VisPuntoDxForm;
import org.clintonhealthaccess.vca.forms.mapeo.VisPuntoDxFormLabels;
...
public class VisPuntoDxActivity extends FragmentActivity implements
    ...
    private static PuntoDiagnostico punto = new PuntoDiagnostico();
    private static PtoDxVisit vispuntodx = new PtoDxVisit();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		username =
				settings.getString(PreferencesActivity.KEY_USERNAME,
						null);
		infoMovil = new DeviceInfo(VisPuntoDxActivity.this);
		punto = (PuntoDiagnostico) getIntent().getExtras().getSerializable(Constants.PUNTO);
        vispuntodx = (PtoDxVisit) getIntent().getExtras().getSerializable(Constants.VISPUNTO);
        roles = getIntent().getExtras().getString(Constants.ROLES);
		
        mPass = ((VcaApplication) this.getApplication()).getPassApp();
        mWizardModel = new VisPuntoDxForm(this,mPass,punto.getIdent());
        ...
        try {
            ...
            if (vispuntodx != null) {
    	        ...
    	        if(tieneValor(vispuntodx.getVisitType())){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey(labels.getVisitType());
                    MessageResource catVisitType = vcaAdapter.getMessageResource(MainDBConstants.catKey + "='" + vispuntodx.getVisitType() + "' and " + MainDBConstants.catRoot + "='CAT_TIPOVISPDX'", null);
                    dato = new Bundle();
                    if(catVisitType!=null) dato.putString(SIMPLE_DATA_KEY, catVisitType.getSpanish());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }
    	        
    	        if(vispuntodx.getVisitDate()!=null){
    		        modifPage = (NewDatePage) mWizardModel.findByKey(labels.getVisitDate());
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, mDateFormat.format(vispuntodx.getVisitDate()));
    	        	modifPage.resetData(dato);
    	        	modifPage.setmVisible(true);
    	        }
    	        else {
    	        ...
    	        if(tieneValor(vispuntodx.getObs())){
    	        	modifPage = (TextPage) mWizardModel.findByKey(labels.getObs());
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, vispuntodx.getObs());
    	        	modifPage.resetData(dato);
    	        	modifPage.setmVisible(true);
    	        }
            }
            ...
            case EXIT:
                ...
                builder.setPositiveButton(this.getString(R.string.yes), new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                    	...
                    	if (punto!=null) arguments.putSerializable(Constants.PUNTO , punto);
    ...
    public void saveData(){
        try {
            ...
            String visitType = datos.getString(this.getString(R.string.visitType));
            String obs = datos.getString(this.getString(R.string.obs));
            
            
            

            if (tieneValor(visitType)) {
            	MessageResource catVisitType = vcaAdapter.getMessageResource(MainDBConstants.spanish + "='" + visitType + "' and " + MainDBConstants.catRoot + "='CAT_TIPOVISPDX'", null);
            	if (catVisitType!=null) vispuntodx.setVisitType(catVisitType.getCatKey());
            } else {
            	vispuntodx.setVisitType(null);
            }
            
            if (tieneValor(obs)) {
                vispuntodx.setObs(obs);
            } else {
            	vispuntodx.setObs(null);
            }
            
            Date visitDate = null;
            try {
            	visitDate = mDateFormat.parse(datos.getString(this.getString(R.string.visitDate)));
            ...
            vispuntodx.setVisitDate(visitDate);

           
            if (vispuntodx.getRecordDate()==null) vispuntodx.setRecordDate(new Date());
            vispuntodx.setRecordUser(username);
            vispuntodx.setDeviceid(infoMovil.getDeviceId());
            vispuntodx.setPasive('0');
            if (vispuntodx.getEstado()==Constants.STATUS_SUBMITTED) {
            	vispuntodx.setEstado(Constants.STATUS_NOT_SUBMITTED);
            }
            else if (vispuntodx.getEstado()==Constants.STATUS_NOT_SUBMITTED) {
            	vispuntodx.setEstado(Constants.STATUS_NOT_SUBMITTED);
            }
            else {
            	vispuntodx.setEstado(Constants.STATUS_NOT_SUBMITTED);
            }
            vispuntodx.setPunto(punto);
            if(vispuntodx.getIdent()==null) {
            	vispuntodx.setIdent(ident);
            	try {
            		vcaAdapter.crearVisPuntoDiagnostico(vispuntodx);
            	}catch (SQLiteException exception) {
            		vispuntodx.setIdent(null);
            	...
            	vcaAdapter.editarVisPuntoDiagnostico(vispuntodx);
            }
            vcaAdapter.close();
            Bundle arguments = new Bundle();
    		if (punto!=null) arguments.putSerializable(Constants.PUNTO , punto);
