...
import org.clintonhealthaccess.vca.activities.mapeo.MenuMuestraActivity;
import org.clintonhealthaccess.vca.activities.mapeo.MuestrasActivity;
import org.clintonhealthaccess.vca.database.VcaAdapter;
import org.clintonhealthaccess.vca.domain.Muestra;
import org.clintonhealthaccess.vca.domain.Localidad;
import org.clintonhealthaccess.vca.forms.mapeo.MuestraForm;
import org.clintonhealthaccess.vca.forms.mapeo.MuestraFormLabels;
...
public class MuestraActivity extends FragmentActivity implements
    ...
    private static Muestra muestra = new Muestra();
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (muestra != null) {
            	...
            	if(muestra.getLocal()!=null){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey(labels.getLocalidad());
                    dato = new Bundle();
                    dato.putString(SIMPLE_DATA_KEY, muestra.getLocal().getName());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }
            	
    	        
    	        if(tieneValor(muestra.getCasa())){
    	        	modifPage = (TextPage) mWizardModel.findByKey(labels.getCasa());
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, muestra.getCasa());
    	        	modifPage.resetData(dato);
    	        	modifPage.setmVisible(true);
    	        }
    	        
    	        if(muestra.getMxDate()!=null){
    		        modifPage = (NewDatePage) mWizardModel.findByKey(labels.getMxDate());
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, mDateFormat.format(muestra.getMxDate()));
    	        	modifPage.resetData(dato);
    	        	modifPage.setmVisible(true);
    	        }
    	        else {
    	        ...
    	        if(muestra.getMxProactiva()!=null){
    	        	modifPage = (IntegerPage) mWizardModel.findByKey(labels.getMxProactiva());
    	            dato = new Bundle();
    	            dato.putString(SIMPLE_DATA_KEY, String.valueOf(muestra.getMxProactiva()));
    	            modifPage.resetData(dato);
    	            modifPage.setmVisible(true);
    	        }
    	        
    	        if(muestra.getMxReactiva()!=null){
    	        	modifPage = (IntegerPage) mWizardModel.findByKey(labels.getMxReactiva());
    	            dato = new Bundle();
    	            dato.putString(SIMPLE_DATA_KEY, String.valueOf(muestra.getMxReactiva()));
    	            modifPage.resetData(dato);
    	            modifPage.setmVisible(true);
    	        }
    ...
    public void updateConstrains(){
    	for (int i = 0; i < mCurrentPageSequence.size(); i++) {
            Page page = mCurrentPageSequence.get(i);
            if (page.getTitle().equals(labels.getMxProactiva())) {
            	String valor = page.getData().getString(IntegerPage.SIMPLE_DATA_KEY);
                if (valor != null && !valor.isEmpty()) {
                	Page pageCambiar2;
                	IntegerPage np2;
                	Integer proactivas = Integer.parseInt(valor);
                	pageCambiar2 = mWizardModel.findByKey(labels.getMxReactiva());
                	np2 = (IntegerPage) pageCambiar2;
                	if(proactivas==0) {
                		np2.setmGreaterOrEqualsThan(1);
                	}
                	else {
                		np2.setmGreaterOrEqualsThan(0);
                	}
    ...
    public void saveData(){
        try {
            ...
            String mxProactiva = datos.getString(this.getString(R.string.mxProactiva));
            String mxReactiva = datos.getString(this.getString(R.string.mxReactiva));
            Date mxDate = null;
            try {
            ...
            muestra.setMxDate(mxDate);
            
            muestra.setLocal(vcaAdapter.getLocalidad(MainDBConstants.name + "= '" + idLocalidad + "'", null));
            if (tieneValor(casa)) {
                muestra.setCasa(casa);
            } else {
            	muestra.setCasa(null);
            }
            
           
            if (tieneValor(mxProactiva)) {
                muestra.setMxProactiva(Integer.valueOf(mxProactiva));
            } else {
            	muestra.setMxProactiva(null);
            }
            
            if (tieneValor(mxReactiva)) {
                muestra.setMxReactiva(Integer.valueOf(mxReactiva));
            } else {
            	muestra.setMxReactiva(null);
            }

           
            if (muestra.getRecordDate()==null) muestra.setRecordDate(new Date());
            muestra.setRecordUser(username);
            muestra.setDeviceid(infoMovil.getDeviceId());
            muestra.setPasive('0');
            if (muestra.getEstado()==Constants.STATUS_SUBMITTED) {
            	muestra.setEstado(Constants.STATUS_NOT_SUBMITTED);
            }
            else if (muestra.getEstado()==Constants.STATUS_NOT_SUBMITTED) {
            	muestra.setEstado(Constants.STATUS_NOT_SUBMITTED);
            }
            else {
            	muestra.setEstado(Constants.STATUS_NOT_FINALIZED);
            }
            
            if(muestra.getIdent()==null) {
            	muestra.setIdent(ident);
            	try {
            		vcaAdapter.crearMuestra(muestra);
            	}catch (SQLiteException exception) {
            		muestra.setIdent(null);
