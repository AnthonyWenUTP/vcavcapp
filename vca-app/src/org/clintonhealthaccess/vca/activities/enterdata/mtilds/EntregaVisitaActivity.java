...
import org.clintonhealthaccess.vca.activities.mtilds.EntregaVisitasActivity;
...
import org.clintonhealthaccess.vca.forms.mtilds.EntregaVisitaForm;
...
public class EntregaVisitaActivity extends FragmentActivity implements
    ...
    private static EntregaVisita visita = new EntregaVisita();
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        visita = (EntregaVisita) getIntent().getExtras().getSerializable(Constants.VISITA);
      ...
      //Aca se recupera los datos de la localidad
        ...
        mWizardModel = new EntregaVisitaForm(this,mPass,visita.getIdent());
        ...
        try {
        	...
        	if (visita != null) {
        		...
        		if(visita.getVisitDate()!=null){
    		        modifPage = (NewDatePage) mWizardModel.findByKey("Fecha de la visita");
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, mDateFormat.format(visita.getVisitDate()));
        		...
        		if(tieneValor(visita.getEntrega())){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey("Se pudo instalar mosquiteros?");
                    dato = new Bundle();
                    dato.putString(SIMPLE_DATA_KEY, visita.getEntrega());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }
        		
        		if(tieneValor(visita.getRazonNoEntrega())){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey("Si no pudo instalar todos los mosquiteros, porque no?");
                    dato = new Bundle();
                    dato.putString(SIMPLE_DATA_KEY, visita.getRazonNoEntrega());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }

        		if(tieneValor(visita.getVisitaRecuperacion())){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey("Requiere una visita de recuperación?");
                    dato = new Bundle();
                    dato.putString(SIMPLE_DATA_KEY, visita.getVisitaRecuperacion());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }
        		
        		
        		if(tieneValor(visita.getObs())){
    	        	modifPage = (TextPage) mWizardModel.findByKey("Observaciones");
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, visita.getObs());
    	        	modifPage.resetData(dato);
    	        	modifPage.setmVisible(true);
    	        }
    ...
    public void updateModel(Page page){
        try{
        	if (page.getTitle().equals("Se pudo instalar mosquiteros?")) {
                if(page.getData().getString(TextPage.SIMPLE_DATA_KEY) != null && page.getData().getString(TextPage.SIMPLE_DATA_KEY).matches(Constants.SI)) {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados tipo cama"), true, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados tipo hamaca"), true, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados suelo"), true, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados otro"), true, null);
                	changeStatus(mWizardModel.findByKey("Si no pudo instalar todos los mosquiteros, porque no?"), false, null);
                }
                else {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados tipo cama"), false, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados tipo hamaca"), false, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados suelo"), false, null);
                	changeStatus(mWizardModel.findByKey("Número de MTILDs entregados/instalados otro"), false, null);
                	changeStatus(mWizardModel.findByKey("Si no pudo instalar todos los mosquiteros, porque no?"), true, null);
                }
