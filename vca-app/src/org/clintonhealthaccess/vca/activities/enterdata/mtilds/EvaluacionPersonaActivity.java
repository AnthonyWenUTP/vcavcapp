...
import org.clintonhealthaccess.vca.forms.mtilds.EvaluacionPersonaForm;
...
public class EvaluacionPersonaActivity extends FragmentActivity implements
    ...
    private static EvaluacionPersona meta = new EvaluacionPersona();
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        meta = (EvaluacionPersona) getIntent().getExtras().getSerializable(Constants.EVMOSQPERSONA);
      ...
      //Aca se recupera los datos de la localidad
        ...
        mWizardModel = new EvaluacionPersonaForm(this,mPass,meta.getIdent());
        ...
        try {
        	...
        	if (meta != null) {
        		...
        		if(tieneValor(meta.getUsoNocheAnterior())){
                    modifPage = (SingleFixedChoicePage) mWizardModel.findByKey("Durmió bajo un mosquitero la noche anterior?");
                    dato = new Bundle();
                    dato.putString(SIMPLE_DATA_KEY, meta.getUsoNocheAnterior());
                    modifPage.resetData(dato);
                    modifPage.setmVisible(true);
                }
        		
        		if(tieneValor(meta.getRazoneNoUso())){
                    modifPage = (MultipleFixedChoicePage) mWizardModel.findByKey("Indique las razones para no hacerlo");
                    dato = new Bundle();
                    List<String> descRazones = new ArrayList<String>();
                    descRazones.add(meta.getRazoneNoUso());
    ...
    public void updateModel(Page page){
        try{
        	if (page.getTitle().equals("Durmió bajo un mosquitero la noche anterior?")) {
                if(page.getData().getString(TextPage.SIMPLE_DATA_KEY) != null && page.getData().getString(TextPage.SIMPLE_DATA_KEY).matches(Constants.SI)) {
                	changeStatus(mWizardModel.findByKey("Indique las razones para no hacerlo"), false, null);
                }
                else {
                	changeStatus(mWizardModel.findByKey("Indique las razones para no hacerlo"), true, null);
                }
