...
import org.clintonhealthaccess.vca.forms.mtilds.EvaluacionForm;
...
public class EvaluacionActivity extends FragmentActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        evaluacion = (Evaluacion) getIntent().getExtras().getSerializable(Constants.EVALUACION);
        roles = getIntent().getExtras().getString(Constants.ROLES);
        
        String mPass = ((VcaApplication) this.getApplication()).getPassApp();
        mWizardModel = new EvaluacionForm(this,mPass,meta.getIdent());
        ...
        try {
        	...
        	if (evaluacion != null) {
        		...
        		if(evaluacion.getVisitDate()!=null){
    		        modifPage = (NewDatePage) mWizardModel.findByKey("Fecha de la evaluación");
    	        	dato = new Bundle();
    	        	dato.putString(SIMPLE_DATA_KEY, mDateFormat.format(evaluacion.getVisitDate()));
        		...
        		if(tieneValor(evaluacion.getObs())){
    ...
    public void updateModel(Page page){
        try{
        	...
        	if (page.getTitle().equals("Existen mosquiteros o MTILDs guardados?")) {
                if(page.getData().getString(TextPage.SIMPLE_DATA_KEY) != null && page.getData().getString(TextPage.SIMPLE_DATA_KEY).matches(Constants.SI)) {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs guardados"), true, null);
                	changeStatus(mWizardModel.findByKey("Porque los MITIDs estan guardados?"), true, null);
                }
                else {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs guardados"), false, null);
                	changeStatus(mWizardModel.findByKey("Porque los MITIDs estan guardados?"), false, null);
                }
                notificarCambios = false;
                onPageTreeChanged();
        	}
        	
        	if (page.getTitle().equals("Existen MTILDs faltantes?")) {
                if(page.getData().getString(TextPage.SIMPLE_DATA_KEY) != null && page.getData().getString(TextPage.SIMPLE_DATA_KEY).matches(Constants.SI)) {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs faltantes"), true, null);
                	changeStatus(mWizardModel.findByKey("Razones por las que hay  MTILDs faltantes"), true, null);
                }
                else {
                	changeStatus(mWizardModel.findByKey("Número de MTILDs faltantes"), false, null);
                	changeStatus(mWizardModel.findByKey("Razones por las que hay  MTILDs faltantes"), false, null);
                }
                notificarCambios = false;
                onPageTreeChanged();
        	}
